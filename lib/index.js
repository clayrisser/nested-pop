...
function nestedPopulateParentObject(parentObject, options) {
  var promise = _.map(options, function(option, key) {
    ...
    if (childModelName.substring(childModelName.length - 1) === 's') childModelName = childModelName.substring(0, childModelName.length - 1);
...
function populateChildModel(childModelName, propertyName, childModel, populate) {
  ...
  if (!sails.models[childModelName]) throw new Error('Model \'' + modelName + '\' does not exits');
  if (!Array.isArray(populate)) populate = [populate.toString()];
